...
one pair of this — refund ↔ chargeback, on card rails. The problem is real
...
and the residual goes to humans — a supported configuration, and the
...
usually right" — a fact about my blocker, not the world — and produced
confident wrong links on exactly the twin-order confusers. Now off by
...
- **Synthetic data.** No public Indian dataset carries linked refund /
  ...
  *lineage* — whether two events refer to one order is a fact, not a guessed
  label — and that the generator spends as much effort on confusers as on
...
- **Stage 3 is unmeasured here** — no credentials on the build machine. The
